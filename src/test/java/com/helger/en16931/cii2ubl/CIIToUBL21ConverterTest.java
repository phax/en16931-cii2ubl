...
public final class CIIToUBL21ConverterTest
{
  ...
  public void testConvertAndValidateAll ()
  {
    for (final String sFilename : MockSettings.TEST_FILES)
    {
      ...
      final Serializable aInvoice = new CIIToUBL21Converter ().convertCIItoUBL (new File ("src/test/resources/cii",
                                                                                          sFilename),
                                                                                aErrorList);
      assertTrue ("Errors: " + aErrorList.toString (), aErrorList.isEmpty ());
      assertNotNull (aInvoice);

      final File aDestFile = new File ("toubl21", FilenameHelper.getBaseName (sFilename) + "-ubl.xml");
      ...
      if (aInvoice instanceof InvoiceType)
      {
        ...
        aResultList = MockSettings.VES_REGISTRY.getOfID (EN16931Validation.VID_UBL_INVOICE_121)
      ...
      else
      {
        ...
        // Validate against EN16931 validation rules
        aResultList = MockSettings.VES_REGISTRY.getOfID (EN16931Validation.VID_UBL_CREDIT_NOTE_121)
                                               .createExecutionManager ()
                                               .executeValidation (ValidationSource.createXMLSource (new FileSystemResource (aDestFile)));
      }
