...
public final class CIIToUBL22ConverterTest
{
  ...
  public void testConvertAndValidateAll ()
  {
    ...
    for (final File aFile : MockSettings.getAllTestFiles ())
    {
      ...
      final File aDestFile = new File ("generated/toubl22/" +
      ...
      if (aInvoice instanceof InvoiceType)
      {
        ...
        final GenericJAXBMarshaller <InvoiceType> aWriter = UBL22Marshaller.invoice ().setFormattedOutput (true);
        aWriter.write (aUBLInvoice, aDestFile);

        // Validate against EN16931 validation rules
        aResultList = ValidationExecutionManager.executeValidation (MockSettings.VES_REGISTRY.getOfID (EN16931Validation.VID_UBL_INVOICE_139),
      ...
      else
      {
        ...
        // Check UBL XSD scheme
        final GenericJAXBMarshaller <CreditNoteType> aWriter = UBL22Marshaller.creditNote ().setFormattedOutput (true);
        aWriter.write (aUBLInvoice, aDestFile);

        // Validate against EN16931 validation rules
        aResultList = ValidationExecutionManager.executeValidation (MockSettings.VES_REGISTRY.getOfID (EN16931Validation.VID_UBL_CREDIT_NOTE_139),
                                                                    ValidationSourceXML.create (new FileSystemResource (aDestFile)));
      }
