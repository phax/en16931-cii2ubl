...
import un.unece.uncefact.data.standard.cii.d25a.udt.DateTimeType;
...
/**
 * CII D25A to UBL 2.5 converter, following the EN 16931:2026 syntax binding.
 *
...
public class CIID25AToUBL25Converter extends AbstractCIIToUBL2026Converter <CIID25AToUBL25Converter>
{
  ...
  private static oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.@Nullable IDType _copyID (@Nullable final IDType aCIIID)
  {
    return copyID (aCIIID, new oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.IDType ());
  }
  ...
  private static oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.@Nullable IDType _extractFirstPartyID (@NonNull final TradePartyType aParty)
  {
    final IDType aID;
  ...
  private static void _extractAllPartyIDs (@NonNull final TradePartyType aParty,
                                           @NonNull final Consumer <? super oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.IDType> aIDConsumer)
  {
    if (canUseGlobalID (aParty))
  ...
  private static void _addPartyID (final oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.@Nullable IDType aUBLID,
                                                ...
                                                @NonNull final PartyType aUBLParty)
  {
    ...
    // UBL 2.5 supports multiple of them
  ...
  private static oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.@Nullable AmountType _copyAmount (@Nullable final AmountType aAmount,
                                                                                                                     @Nullable final String sDefaultCurrencyCode)
  {
    return copyAmount (aAmount,
                       new oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.AmountType (),
  ...
  private void _convertPaymentMeans (@NonNull final HeaderTradeSettlementType aHeaderSettlement,
                                     ...
                                     @NonNull final Consumer <oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.IDType> aSellerIDHandler,
                                     ...
                                     @NonNull final ErrorList aErrorList)
  {
    ...
    if (bIsBG17 || bIsBG18 || bIsBG19 || isPaymentMeansCodeOtherKnown (sTypeCode))
      aPaymentMeansSuccessHandler.accept (aUBLPaymentMeans);
    else
      aErrorList.add (buildError (null,
                                       ...
                                       @NonNull final ErrorList aErrorList)
  {
    ...
    {
      // CII D25A: InvoiceReferencedDocument is a 0..n element, matching BG-3
      for (final ReferencedDocumentType aRD : aHeaderSettlement.getInvoiceReferencedDocument ())
      {
    ...
    // BG-13 DELIVERY INFORMATION
    {
      ...
      {
        final oasis.names.specification.ubl.schema.xsd.commonaggregatecomponents_25.LocationType aUBLDeliveryLocation = new oasis.names.specification.ubl.schema.xsd.commonaggregatecomponents_25.LocationType ();
        boolean bUseLocation = false;

        // BT-71/BT-71-1 Deliver to location identifier
        final oasis.names.specification.ubl.schema.xsd.commonbasiccomponents_25.IDType aUBLID = _extractFirstPartyID (aShipToParty);
        if (aUBLID != null)
        {
    ...
    for (final SupplyChainTradeLineItemType aLineItem : aSCTT.getIncludedSupplyChainTradeLineItem ())
    {
      ...
      if (aLineProduct != null)
      {
        ...
        // CII D25A: Description is a 0..n element
        for (final TextType aDescription : aLineProduct.getDescription ())
          ifNotNull (copyName (aDescription, new DescriptionType ()), aUBLItem::addDescription);

        // BT-153 Item name
        if (aLineProduct.hasNameEntries ())
          ifNotNull (copyName (aLineProduct.getNameAtIndex (0), new NameType ()), aUBLItem::addName);
