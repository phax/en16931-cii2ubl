...
* The BT-3 (Invoice type code) sets are now edition specific. EN 16931:2026 uses the normative EN 16931 code list "v17b - used from 2026-05-15", EN 16931:2017 continues to follow the EN 16931 validation artefacts, against which this project validates its UBL 2.1 output. The two differ in exactly one code: the artefacts additionally accept `81` on an Invoice, while every version of the code list has it as a Credit Note only, so a document with BT-3 = `81` becomes an Invoice under 2017 and a CreditNote under 2026
